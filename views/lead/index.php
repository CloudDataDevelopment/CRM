...
<div class="lead-index">
    <div class="leads-wrapper">
        ...
        <div class="leads-header">
            <div>
                <div class="breadcrumb-custom">
                    ...
                    <span class="current">Gestor de leads</span>
                </div>
        ...
        <!-- MÉTRICAS -->
        <div class="row g-2 mb-2" id="metricas-container">
            <?php foreach ($metricas as $index => $metrica): ?>
                    ...
                    <div class="card stat-card stat-card-<?= $metrica['class'] ?> dashboard-card">
                        <div class="card-body d-flex align-items-center">
                            ...
                            <div class="flex-grow-1">
                                <div class="stat-number" data-metrica="<?= $index ?>"><?= $metrica['value'] ?></div>
                ...
                <div class="card table-card">
                    <div class="card-header">
                        <div class="header-left">
                            ...
                            <span class="badge bg-primary ms-2" id="leads-count-badge"><?= $dataProvider ? $dataProvider->getTotalCount() : 0 ?></span>
                        </div>
                                    ...
                                    <?php if (!empty($leads)): ?>
                                        <?php foreach ($leads as $lead): ?>
                                            <tr class="lead-row" data-id="<?= $lead->id_lead ?>">
                                                <td class="td-nombre"><strong><?= Html::encode($lead->name . ' ' . $lead->lastname) ?></strong></td>
                                            ...
                                            <div class="funnel-block-trapezoid" style="width: <?= $etapa['width'] ?>%; background: <?= $etapa['color'] ?>; border-left: <?= $etapa['border_size'] ?> solid transparent; border-right: <?= $etapa['border_size'] ?> solid transparent; border-top: 0px solid transparent;">
                                                <div class="funnel-block-content-trapezoid">
                                                    ...
                                                    <div class="funnel-stats-trapezoid">
                                                        <span class="funnel-count-trapezoid" data-etapa="<?= $etapa['label'] ?>"><?= $etapa['count'] ?></span>
                                                    </div>
                                                </div>
...
function inicializar() {
    ...
    function openPanel(leadId) {
        ...
        fetch(viewModalUrl + '?id=' + leadId)
            ...
            .catch(function(error) {
                ...
                panelContent.innerHTML = '<div class="text-center text-danger py-5"><i class="fas fa-exclamation-triangle fa-3x"></i><p>Error al cargar</p><button class="btn btn-secondary btn-sm mt-3" data-panel-close>Cerrar</button></div>';
            });
    }
    ...
    function openEditPanel(leadId) {
        ...
        fetch('<?= Url::to(['lead/update']) ?>?id=' + leadId + '&modal=1')
            ...
            .catch(function(error) {
                console.error('❌ Error:', error);
                panelContent.innerHTML = '<div class="text-center text-danger py-5"><i class="fas fa-exclamation-triangle fa-3x"></i><p>Error al cargar el formulario</p><button class="btn btn-secondary btn-sm mt-3" data-panel-close>Cerrar</button></div>';
            });
    }
    ...
    // ============================================
    window.onLeadUpdated = function(data) {
        console.log('🔄 Actualizando lead en tiempo real:', data);

        if (!data || !data.id_lead) {
            console.warn('⚠️ Datos incompletos');
            return;
        }

        var $fila = document.querySelector('.lead-row[data-id="' + data.id_lead + '"]');
        if (!$fila) {
            console.warn('⚠️ Fila no encontrada para id:', data.id_lead);
            return;
        }

        // 1️⃣ Actualizar NOMBRE
        if (data.name) {
            var tdNombre = $fila.querySelector('.td-nombre');
            if (tdNombre) {
                tdNombre.innerHTML = '<strong>' + escapeHtml(data.name) + '</strong>';
            }
        }

        // 2️⃣ Actualizar TELÉFONO
        if (data.phone !== undefined) {
            var tdTel = $fila.querySelector('.td-telefono');
            if (tdTel) {
                var html = '<div class="d-flex align-items-center gap-2">';
                html += '<span>' + escapeHtml(data.phone) + '</span>';
                if (data.phone) {
                    var waNumber = String(data.phone).replace(/[^0-9]/g, '');
                    html += '<a href="https://wa.me/' + waNumber + '" target="_blank" class="text-success" title="WhatsApp"><i class="fab fa-whatsapp"></i></a>';
                }
                html += '</div>';
                tdTel.innerHTML = html;
            }
        }

        // 3️⃣ Actualizar ESTADO
        if (data.status_name) {
            var tdEstado = $fila.querySelector('.td-estado');
            if (tdEstado) {
                var badgeClass = getBadgeClassFromStatus(data.status_name);
                tdEstado.innerHTML = '<span class="badge bg-' + badgeClass + '">' + escapeHtml(data.status_name) + '</span>';
            }
        }

        // 4️⃣ Actualizar OBSERVACIONES
        if (data.comments !== undefined) {
            var tdComments = $fila.querySelector('.td-comments');
            if (tdComments) {
                var texto = data.comments || '';
                if (texto.length > 50) {
                    texto = texto.substring(0, 50) + '...';
                }
                tdComments.textContent = texto;
            }
        }
        ...
        console.log('✅ Lead actualizado en la tabla');
    };
    ...
    // Helper: badge según estado
    function getBadgeClassFromStatus(statusName) {
        var map = {
            'Nuevo': 'primary',
            'Contactado': 'info',
            'Procesando': 'warning',
