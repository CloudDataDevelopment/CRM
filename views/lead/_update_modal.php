...
if (!$model) {
    echo '<div class="edit-panel-content"><div class="edit-panel-header"><div class="header-title"><i class="fas fa-inbox text-muted"></i> Lead no encontrado</div><button type="button" class="btn-close-panel" onclick="closePanel()"><i class="fas fa-times"></i></button></div><div class="edit-panel-body"><div class="edit-panel-message"><i class="fas fa-inbox text-muted"></i><p>Lead no encontrado</p><button class="btn-message btn-message-secondary" onclick="closePanel()"><i class="fas fa-times"></i> Cerrar</button></div></div></div>';
    return;
}
?>
...
<div class="edit-panel-content" id="edit-panel-content">
    ...
    <div class="edit-panel-body">
        ...
        <?= $form->field($model, 'email')->textInput(['class' => 'form-control form-control-sm', 'id' => 'lead-email'])->label('Email') ?>
        ...
        <?= $form->field($model, 'comments')->textarea(['rows' => 3, 'class' => 'form-control form-control-sm', 'placeholder' => 'Observaciones...'])->label('Observaciones') ?>
    ...
    .then(function(data) {
        ...
        if (container && newContent) {
            ...
            setTimeout(function() {
                ...
                if (a) { a.style.transition = 'opacity .5s'; a.style.opacity = '0'; setTimeout(function() { if (a) a.style.display = 'none'; }, 500); }
            }, 3000);

            // 🔥 Notificar al padre para actualización en tiempo real
            var form2 = document.getElementById('update-lead-form');
            if (form2 && window.parent && typeof window.parent.onLeadUpdated === 'function') {
                var idMatch = form2.getAttribute('action').match(/id=(\d+)/);
                var nameEl = document.querySelector('#lead-name');
                var lastnameEl = document.querySelector('#lead-lastname');
                var phoneEl = document.querySelector('#lead-phone');
                var emailEl = document.querySelector('#lead-email');
                var statusEl = document.querySelector('#lead-id_status');
                window.parent.onLeadUpdated({
